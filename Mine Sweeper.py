...
class MainWindow(Q.QMainWindow):
    ...
    def create_buttons(self, rows, cols, container):
        ...
        for i in range(rows):
            ...
            for j in range(cols):
                ...
                button.clicked.connect(lambda checked, row = i, col = j: self.space_clicked(row, col))
    ...
    def is_valid_index_in_mines_list(self, row, col):
        #this is short circuits
        #if row is invalid, it won't check col.
        return 0 <= row < len(self.mine_list) and 0 <= col < len(self.mine_list[row])

    def get_mines_counts(self, row, col):
        mine_count = 0
        for i in range(-1, 2):
            for j in range(-1, 2):
                target_row = row + i
                target_col = col + j
                if self.is_valid_index_in_mines_list(target_row, target_col) and self.mine_list[target_row][target_col].is_mine:
                    mine_count +=1
        return mine_count
